...
func TestIntrospect_ValidKey(t *testing.T) {
	srv, _ := mockCP(t, func(req introspectRequest) Result {
		return Result{Valid: true, Account: "alice@vulos.org", Scopes: []string{"documents.read"}, Products: []string{"office", "mail"}}
	})
	...
	if !res.HasProduct(ProductOffice) {
		t.Fatal("expected office product")
	}
	if res.HasProduct("talk") {
		t.Fatal("did not expect talk product")
	}
	if !res.HasScope("documents.read") {
		t.Fatal("expected documents.read scope")
	}
}
...
func TestIntrospect_CachesResults(t *testing.T) {
	srv, calls := mockCP(t, func(req introspectRequest) Result {
		return Result{Valid: true, Account: "bob@vulos.org", Products: []string{"office"}}
	})
	intro := NewIntrospectorWithClient(Config{BaseURL: srv.URL}, srv.Client())
...
func TestIntrospect_CacheExpires(t *testing.T) {
	srv, calls := mockCP(t, func(req introspectRequest) Result {
		return Result{Valid: true, Account: "carol@vulos.org", Products: []string{"office"}}
	})
	intro := NewIntrospectorWithClient(Config{BaseURL: srv.URL}, srv.Client())
...
func TestIntrospect_SendsServiceToken(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(Result{Valid: true, Products: []string{"office"}})
	}))
	t.Cleanup(srv.Close)
